...
description: 在 PHP 7 之前，PHP 有两种使用正则表达式的方法：PCRE（兼容 Perl 的 preg_* 函数）和 POSIX（扩展 ereg_* 函数）。
  每个函数族使用的正则表达式语法略有不同。幸运的是，ereg_* 函数在 PHP 7 中已被移除，这种混淆已经不存在了。
head:
  - - meta
    - name: keywords
      content: PHP 正则表达式入门,Unicode,POSIX,Perl,preg,ereg,PCRE,正则表达式,PHP最佳实践,PHP标准规范,PHP Standards Recommendations,PSR,PHP,noob-coder,菜鸟码农
---
