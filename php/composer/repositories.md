...
- **`lists`**（必需，对象）— 此仓库提供的所有过滤列表的名称映射，映射到一个描述该列表的对象。该对象当前包含一个 `enabled` 标志（当列表可用时设置为 `true`），采用这种结构是为了将来可以在不破坏传输格式的情况下添加每个列表的元数据。
- **`summary-url`**（可选，字符串）— 一个 URL（绝对路径或根相对路径），返回列表名称 → 包名称 → 版本约束的紧凑映射。配置后，Composer 会在 `composer install`、`composer update` 和 `composer audit` 期间获取此端点，并使用它来跳过无法匹配任何活动列表的包的每个包元数据获取。
  ...
  Composer 在每次安装/更新/审计时通过 `If-Modified-Since` 重新验证缓存的摘要，使用与包元数据相同的磁盘仓库缓存。

- **`api-url`**（可选，字符串）— 一个 URL（绝对路径或根相对路径），接受包含相关包 PURL 和已配置列表名称的 POST 请求，并直接返回匹配的过滤条目。设置后，Composer 会使用 `api-url` 而不是 `summary-url` 和每个包的元数据来进行过滤；这在摘要太大而无法完整提供时非常有用。如果同时公布了 `summary-url` 和 `api-url`，则 `api-url` 优先，`summary-url` 将被忽略。实现者应该注意，Composer 可能会提交大量（几百个是正常的）包名。你可以决定限制这个列表，只返回前 1000 个或其他你决定的数量，但一定要以某种方式限制它。
  ...
  每个条目具有与每个包元数据过滤条目相同的结构，另外增加了 `package` 字段（因为一个响应涵盖多个包）。`api-url` POST 请求不会在客户端缓存，因为每个请求体都不同。
...
在 `composer.json` 文件中，仓库定义上的 `filter` 键用于控制在审计报告和版本拦截中，哪些由该仓库公布的列表会被采纳。

设置 `filter: false` 可以选择不采纳此仓库公布的所有列表：
...
将 `filter` 设置为一个对象，列出应该从此仓库中跳过的已公布列表名称。
...
仓库级别的 `filter` 只能缩小此仓库贡献的范围；它不能启用在 [`config.policy`](config.md#policy) 全局配置中未配置的列表。
