...
> [!WARNING] 注意 ⚠️
> 该功能存在严重的技术限制。引用（reference）只会改变被检出的提交（commit）；它是在安装时作为一种底层覆盖应用的，依赖求解器（dependency solver）永远看不到它。因此：
>
> - 该包的 `composer.json` 元数据（其自身的 `require` 条目、自动加载规则等）是从你所指定的分支的当前状态读取的，而不是从你固定的那个提交读取的。因此，Composer 解析出的依赖关系可能与该提交处所声明的依赖关系不同。
> - 该覆盖只对源码安装（source install）可靠。分发安装（dist install）只有在分发文件来自能够为任意提交构建归档（如 GitHub）的源时，才能遵循该引用；否则在下载文件时，固定的提交会被忽略。
>
> 因此，你只应在开发期间将其作为临时方案，用于缓解临时性问题，直到你能切换到带标签的发布版本。Composer 团队不主动支持该功能，也不会接受与之相关的缺陷报告。
